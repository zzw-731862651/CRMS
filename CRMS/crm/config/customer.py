...
class CustomerConfig(StarkConfig):   #要是想要实现自己的方法，就自己写视图函数，先用自己的，后用父类的
...
class PublicCustomerConfig(StarkConfig):    #公户配置文件
    list_display = [
        'name',
        'qq',
        get_choice_text('status', '状态'),
...
class PrivateCustomerConfig(StarkConfig):     #私户配置文件
    def display_follow(self,row=None, header=False):
        if header:
            return '跟进记录'
